...
subString start end str
  ...
  | otherwise = subString (start - 1) (end - 1) (tail str)
...
lowerAndGreater n list = let (l, g) = go n list in fillTemplate n l g
  where
    ...
    go y (x : xs) = (lower, greater)
      where
        (l, g) = go n xs
        lower = if x < y then l + 1 else l
        greater = if x > y then g + 1 else g